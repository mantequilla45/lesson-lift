...
import { Search, Plus, Monitor, LayoutGrid, Trash2, Loader2 } from "lucide-react";
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      {loading ? (
        <>
          ...
          <div className="-mt-4 h-5 w-32 rounded-full animate-pulse" style={{ backgroundColor: "var(--j-tint-2)" }} />
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="bg-(--j-card) rounded-2xl overflow-hidden border animate-pulse" style={{ borderColor: "var(--j-line)" }}>
                <div className="aspect-video" style={{ backgroundColor: "var(--j-tint)" }} />
                <div className="p-4 space-y-2.5">
                  <div className="h-4 rounded-full w-3/4" style={{ backgroundColor: "var(--j-tint-2)" }} />
                  <div className="h-3 rounded-full w-1/2" style={{ backgroundColor: "var(--j-tint)" }} />
                </div>
              </div>
            ))}
          </div>
