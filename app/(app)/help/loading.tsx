...
//
// The sidebar and top bar come from app/(app)/layout.tsx and are NOT remounted
// between these routes, so this renders only the page body. Mounting a second
// AppShellV2 here would draw a whole sidebar inside the real one. HelpView
// declares the fixed variant and the content well itself on arrival.
export default function Loading() {
  return (
    <div className="px-4 sm:px-6 lg:px-10 pb-4 sm:pb-10 grow min-h-0">
      <div
      ...
      >
        <div className="lg:border-r p-3 space-y-3" style={{ borderColor: "var(--j-line)" }}>
          <div className="h-10 rounded-2xl" style={{ backgroundColor: "var(--j-tint)" }} />
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="space-y-2 px-1 py-2">
              <div className="h-3.5 w-2/3 rounded" style={{ backgroundColor: "var(--j-tint)" }} />
              <div className="h-3 w-full rounded" style={{ backgroundColor: "var(--j-tint)" }} />
              <div className="h-3 w-1/3 rounded" style={{ backgroundColor: "var(--j-tint)" }} />
            </div>
          ))}
        </div>
        ...
        <div className="hidden lg:block p-6 space-y-3">
          <div className="h-4 w-1/3 rounded" style={{ backgroundColor: "var(--j-tint)" }} />
          <div className="h-3 w-1/4 rounded" style={{ backgroundColor: "var(--j-tint)" }} />
        </div>
      </div>
    </div>
  );
